...
compiler::Item_expr::Item_expr(const uint32_t& lineno) : Item(lineno) {}

compiler::Item_expr_cond::Item_expr_cond(const uint32_t& lineno,
                                         Item_expr* const item_expr)
    : Item_expr(lineno), expr(item_expr) {}

compiler::Item_expr_comma::Item_expr_comma(const uint32_t& lineno)
    : Item_expr(lineno) {}

compiler::Item_expr_binary::Item_expr_binary(const uint32_t& lineno,
                                             const binary_type& type,
                                             Item_expr* const lhs,
                                             Item_expr* const rhs)
    : Item_expr(lineno), type(type), lhs(lhs), rhs(rhs) {}

compiler::Item_expr_unary::Item_expr_unary(const uint32_t& lineno,
...
std::string compiler::Item_expr_unary::print_result(const uint32_t& indent,
  ...
  oss << " Unary Expression with type " << termcolor::bright_blue
      << compiler::to_string(get_unary_type()) << termcolor::reset << std::endl;
