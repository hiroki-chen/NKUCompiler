...
compiler::ir::BranchIR
compiler::Item_expr::eval_cond_helper(
    compiler::ir::IRContext* const ir_context,
    std::vector<compiler::ir::IR>& ir_list) const
{
    compiler::ir::BranchIR branch_ir;
    ir_list.emplace_back(
        compiler::ir::op_type::CMP,
        new compiler::ir::Operand(),
...
compiler::ir::Operand*
compiler::Item_expr::eval_runtime_helper(
    compiler::ir::IRContext* const ir_context,
    std::vector<compiler::ir::IR>& ir_list) const
{
    throw compiler::unsupported_operation("Cannot evaluate this expression at runtime!");
}

compiler::ir::Operand*
compiler::Item_expr::eval_runtime_helper(compiler::ir::IRContext* const ir_context) const
{
    throw compiler::unsupported_operation("Cannot evaluate this expression at runtime!");
}

compiler::ir::BranchIR
compiler::Item_expr::eval_cond(
    compiler::ir::IRContext* const ir_context,
    std::vector<compiler::ir::IR>& ir_list) const
{
    stack.push_back(static_cast<compiler::Item*>(const_cast<compiler::Item_expr*>(this)));
    try {
        const compiler::ir::BranchIR branch_ir = eval_cond_helper(ir_context, ir_list);
        stack.pop_back();
        return branch_ir;
    ...
    std::vector<compiler::ir::IR>& ir_list) const
{
    stack.push_back(static_cast<compiler::Item*>(const_cast<compiler::Item_expr*>(this)));
    try {
        compiler::ir::Operand* const operand = eval_runtime_helper(ir_context, ir_list);
        stack.pop_back();
        return operand;
    } catch (const std::exception& e) {
        stack.pop_back();
...
compiler::ir::Operand*
compiler::Item_expr::eval_runtime(compiler::ir::IRContext* const ir_context) const
{
    stack.push_back(static_cast<compiler::Item*>(const_cast<compiler::Item_expr*>(this)));
    try {
...
compiler::ir::Operand*
compiler::Item_expr_binary::eval_runtime_helper(compiler::ir::IRContext* const ir_context) const
{
    ...
    case compiler::binary_type::ADD_TYPE: {
        const double lhs = compiler::ir::convert_from_string(expr_left->get_value());
        const double rhs = compiler::ir::convert_from_string(expr_right->get_value());
        const double res = lhs + rhs;
        const compiler::ir::var_type type = compiler::ir::get_type_priority(
...
compiler::Item_expr_binary::eval_runtime_helper(
    ...
    std::vector<compiler::ir::IR>& ir_list) const
{
    // First check the optimization level. If so, we calculate the expression at runtime.
    if (opt_level > 0) {
        try {
