    ...
    const
{
    try {
        ...
        // If branches cannot be evaluated at runtime, we should create IRs for each branch.
        ...
        const auto& symbol_table = ir_context_if.get_symbol_table().get_symbol_table();
        // Traverse the symbol table.
        for (auto iter = symbol_table.cend(); iter != symbol_table.cbegin(); iter--) {
